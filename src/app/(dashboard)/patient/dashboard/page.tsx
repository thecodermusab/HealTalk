  ...
  const quickActions = [
    {
      ...
      iconClass: "text-primary group-hover:text-background"
    },
    {
      icon: MessageCircle,
      label: "Send Message",
      href: "/patient/dashboard/messages",
      bgClass: "bg-secondary/10 group-hover:bg-secondary",
      iconClass: "text-secondary group-hover:text-background"
    },
    {
      icon: Heart,
      ...
      iconClass: "text-accent group-hover:text-background"
    },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {quickActions.map((action) => (
            <Link key={action.label} href={action.href}>
              <button className="w-full p-6 bg-card border border-border rounded-xl hover:border-primary hover:shadow-lg transition-all text-left group">
                <div className="flex items-center gap-4">
                  <div className={`w-14 h-14 rounded-lg flex items-center justify-center transition-colors ${action.bgClass}`}>
                    <action.icon className={`transition-colors ${action.iconClass}`} size={28} />
                  </div>
        ...
        {nextAppointment && (
          <div className="bg-gradient-to-r from-primary via-primary/90 to-accent rounded-2xl p-8 mb-8 text-background">
            <div className="flex items-start justify-between mb-4">
              <div>
                <div className="text-sm text-background/80 mb-1">Next Appointment</div>
            ...
            <div className="flex gap-3">
              ...
              <Button variant="outline" className="border-background text-background hover:bg-card/20">
                Reschedule
              </Button>
            </div>
          </div>
